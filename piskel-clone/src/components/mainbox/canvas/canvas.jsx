...
import {
  colors, tools, mouseEvents, frameSizes, penSizes,
} from '../../../assets/data';
import './canvas.scss';
    ...
    if (updatedFrames[id].color !== color) {
      ...
      if (drawPenSize.length) {
        drawPenSize.forEach((key) => {
          ...
          const newArr = [...updatedFrames];
          newArr.splice(key.id, 1, { ...newArr[key.id], color });
          updatedFrames = changeUpdated(newArr);
        });
      }
    }
  };
    ...
    const addToBucket = (part) => {
      updatedFrames.forEach((val) => {
        if (val.id === part.id) {
          ...
          if (row <= matrixLength - 2) {
            adjacent.push(updatedFrames[(row + 1) * matrixLength + column].id);
          }
          if (column <= matrixLength - 2) {
            adjacent.push(updatedFrames[row * matrixLength + column + 1].id);
          }
          adjacent.forEach((pixel) => {
            if (bucketList.indexOf(updatedFrames[pixel]) < 0
              && updatedFrames[pixel].color === part.color) {
...
Canvas.propTypes = {
  ...
  frame: PropTypes.shape({ id: PropTypes.number, array: PropTypes.array }),
  onSetUpdatedFrame: PropTypes.func.isRequired,
